...
def verifyValidation(self, target, value):
     ...
     self.driver.waitForAttribute(target + '@class', expectedClassValue.strip())
     target_elem = self.driver._find_target(target)
     ActionChains(self.driver).move_to_element(target_elem).perform()
     assert self.driver.getText('//[@id="validationMsg"]').strip() == expectedValidationMsg.strip()
